...
const start = async () => {
  try {
    ...
    console.log(`[v0] MCP Server running on http://localhost:${config.port}`)
    console.log(`[v0] MCP endpoint: http://localhost:${config.port}/mcp`)
    console.log(`[v0] Backend URL: ${config.backendUrl}`)
  } catch (err) {
